...
room.onPlayerJoin = function(player) {	
	...
	whisper("🌐      Bem-Vindo a HSBR      🌐", player.id, 0x61ddff, "bold", 0);
	...
	if (map == "RSR") {
		...
		if (powerShotMode == true) {
			if (game.powershotCounter > 100) {
				...
				room.sendAnnouncement("POWERSHOT!", player.pm, 0x33dddd, "bold", 1);
			}
	...
	if (message.startsWith("!")) {
		...
		if (args[0] == "admin" && args.length == 1 && allowPublicAdmin == true) {
			...
			if (superAdmins.indexOf(player.id) > -1) {
				...
				roomPassword = args[1];
				announce("Senha modificada por: " + player.name);
			}
			else {
			...
			if (superAdmins.indexOf(player.id) > -1) {
				...
				roomPassword = null;
				announce("Senha limpa por: " + player.name);
			}
			else {
			...
			if (superAdmins.length > 0) {
				...
				superMsg = "Não tem super admins presente";
			}
			...
			if (args.length > 1) {
				...
				players.forEach(function(pmPlayer) {
					if (pmPlayer.name === args[0] || pmPlayer.name === args[0].replace(/_/g, ' ')) {
						...
						whisper("[DM] " + player.name + ": " + pmMsg, pmPlayer.id, 0xff20ff, "normal", 1);
						pmSent = true;					
					}
				});
				if (pmSent == false) {
...
function displayHelp(id, selection) {
	if (selection == null) {
		whisper("Commands: !rr, !bb, !setpassword, !clearpassword, !super, !clearbans, !swap, t [team chat msg] ", id, null, "small");
	}
}
		...
		if (team == 1) {
			if (game.lastKickerTeam == 1) { //if goal type is goal
				...
				if (game.secondLastKickerTeam == 1 && game.lastKickerId != game.secondLastKickerId) { // if assist is from teammate
					assister = " (Assistencia: " + game.secondLastKickerName + ")";
...
function extraTime() {
	...
	announce("Acrescimos: " + extraSeconds + " Segundos", null, null, null, 1);
}
...
if (players.length >= 1 && room.getScores() == null) {
room.startGame();
}
}
...
players.forEach(p => {
if (p.team === 0) {
if (red <= blue && red < maxTeamSize) {
room.setPlayerTeam(p.id, 1);
red++;
} else if (blue < maxTeamSize) {
...
if (players.length == 0 && room.getScores() != null) {
room.stopGame();
}
